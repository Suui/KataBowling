...
TEST_CASE("Testing Frame Scores", "[score][frame]")
{
	...
	{
		auto TestFrame = Frame(0, 0);
		CHECK(TestFrame.GetScore() == 0);
	}

	SECTION("5 Frame Score")
	{
		auto TestFrame = Frame(2, 3);
		CHECK(TestFrame.GetScore() == 5);
	}
}


TEST_CASE("Testing whether frame is a Strike, Spare or a Last Frame", "[score][frame]")
{
	auto SpareFrame = Frame(0, 10);
	auto StrikeFrame = Frame(10, 0);
	auto LastFrame = Frame(2, 2, 2);
